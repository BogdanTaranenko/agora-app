...
class AgoraRTCView extends Component<Props> {
  ...
  componentWillMount () {
    ...
    RtcEngine.on('tokenPrivilegeWillExpire', () => {
      console.log("[RtcEngine] tokenPrivilegeWillExpire");
      ...
      axios.get(
        ...
        RtcEngine.renewToken(data.token);
      })
    })
  ...
  componentDidMount () {
    ...
    RtcEngine.joinChannel(this.props.channelName, this.props.uid, this.props.token);
...
export default class AgoraRTCViewContainer extends React.Component {
  static navigationOptions = {
    header: null,
  };
  render () {
